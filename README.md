...
対応しているパッケージマネージャーは `dnf` (Fedora)、`apt-get` (Debian / Ubuntu)、`pacman` (Arch) 。Debian / Ubuntuでは fzf と Neovim を GitHub Releases の最新版バイナリから導入し、取得できない場合は apt 版へフォールバックする。Archで `paru` が無い場合は、AURから `paru` を導入するか確認してから進む。
